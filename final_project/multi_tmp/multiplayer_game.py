...
class MultiplayerGameWindow(GameWindow):
    ...
    def show_results(self):
        ...
        for idx, pid in enumerate(ordered_players):
            ...
            delay = idx*2000
            def show_label(text=display_text, color=fg_color, font=font_style):
                tk.Label(result_window, text=text, font=font, bg="#f8f9fa", fg=color).pack(pady=5)
        ...
        def show_back_button():
            tk.Button(
                ...
                bg="#95a5a6",
                fg="#ffffff",
                command=lambda: [result_window.destroy(), self.back_to_menu()]
            ).pack(pady=30)
        
        result_window.after(len(self.finish_order) * 500 + 500, show_back_button)
        '''
